      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {activeSemester ? (
              <>Current semester: <span className="font-medium">{activeSemester.name}</span></>
            ) : (
              'No active semester'
            )}
          </p>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <CardContent>
            ...
            {activeSemester?.addDropDeadline && (
              <p className="text-xs text-muted-foreground">
                Deadline: {new Date(activeSemester.addDropDeadline).toLocaleDateString()}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">GPA: 3.5 (estimated)</p>
          </CardContent>
        </Card>
      </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {course.schedule?.day} {course.schedule?.startTime}-{course.schedule?.endTime} • {course.schedule?.room || 'TBA'}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <Badge variant="outline">{course.creditHours} credits</Badge>
                      <p className="text-xs text-muted-foreground mt-1">{course.instructor?.name || 'Staff'}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {new Date(activeSemester.addDropDeadline).toLocaleDateString('en-US', {
                          weekday: 'short',
                          month: 'short',
                          day: 'numeric',
                        })}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {new Date(activeSemester.finalExamStart).toLocaleDateString('en-US', {
                          weekday: 'short',
                          month: 'short',
                          day: 'numeric',
                        })}
            ...
            <CardContent className="pt-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Visit the course catalog to explore offerings or contact your academic advisor.
                  </p>
                </div>
              </div>
            </CardContent>
